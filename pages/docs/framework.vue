...
AppPage
  ...
  main.text(slot="text" role="text")
    article.paragraph
      ...
      p 티도리 프레임워크에서는 모던 자바스크립트 프레임워크인 #[b 뷰(Vue.js)]와 #[b 리액트(React.js)]를 지원합니다. #[b 컴포넌트(Component)]를 만들고 포함시켜 템플릿에서 사용할 수 있습니다. 컴포넌트는 사용하고 싶을 떄 사용하면 되지만 몇 가지 주의사항이 있으니 알아보도록 합시다.
    ...
    article.paragraph
      h2(id="리소스") 리소스
      p #[b 컴포넌트]에서 에셋을 포함하려면 #[b 속성 바인딩] 기능을 포함하거나, 정적파일을 포함하듯 포함시키면 됩니다. 우리가 일반적으로 #[b skin.html] 에 #[b ./images/logo.png] 와 같은 형식으로 포함하면 티스토리는 자동으로 에셋의 경로를 #[b CDN(Content Delivery Network)]으로 변경합니다. 컴포넌트에서 정적파일 또는 에셋을 불러오는 경우 자바스크립트를 통한 포함이므로 #[b 스킨 내부에서 별도로 경로 변환을 하지 않습니다.] 따라서 따로 #[b PUBLIC PATH] 설정을 해줄 필요가 있습니다.
      p
  ...
  footer.footer(slot="footer" role="footer")
    div.arrows
      div.left: nuxt-link(to="/docs/example") #[i.fas.fa-angle-left] 티스토리 예제
      div.right: nuxt-link(to="/docs/distribute/") 빌드 및 배포 #[i.fas.fa-angle-right]
</template>
