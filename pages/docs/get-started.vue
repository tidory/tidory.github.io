...
app-page(:active="$store.state.menu.getStarted.page")
  ...
  main.text(slot="text" role="text")
    ...
    article.paragraph
      ...
      p #[b 티도리(Tidory)]는 티도리 프레임워크와 상호작용하기 위한 #[b CLI(Command Line Interface)] 환경에서 실행하는 #[b NPM(Node Package Manager)] 패키지입니다. 다음과 같은 명령을 사용하여 템플릿을 받아온 뒤, 프로젝트 구동을 위한 #[b NPM] 패키지들을 설치하십시오.
      pre
        code.lang-bash
  ...
  footer.footer(slot="footer" role="footer")
    div.arrows
      ...
      div.right: a(:href="$store.state.menu.directory.href") {{ $store.state.menu.directory.label }} #[i.fas.fa-angle-right]
</template>
