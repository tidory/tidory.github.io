...
app-page(:active="$store.state.menu.advanced.package.page")
  ...
  main.text(slot="text" role="text")
    ...
    article.paragraph
      ...
      p 티도리 패키지는 #[b 취급]을 #[b NPM 패키지]로 취급하기 때문에 #[b package.json] 이 필수적입니다. #[b 티도리 패키지]는 다른 패키지와는 달리 테스트하기 어려운 점이 있는데, 우리가 기존에 사용하던 #[b tidory-cli] 를 사용하면 #[b 개발 서버, 프리뷰 서버]를 똑같이 이용할 수 있습니다. 티도리에 등록된 #[a(href="https://github.com/tidory/package" target="_blank") package] 레포지토리를 사용하면 패키지 개발을 조금 더 쉽게할 수 있습니다.
      ...
      pre(data-label="package.json")
        code.lang-json {{ pkg }}
      p #[b 패키지 개발 전용]으로 제공된 #[b 레포지토리]에는 #[b env, tidory.config.js] 등 개발을 위한 파일이 포함되어 있는데, #[b 티도리 프로젝트 템플릿]에서 포함할 때는 의미가 없으므로 #[b files] 배열에 포함시킬 필요가 없습니다. 하지만 #[b 컴포넌트] 또는 #[b 템플릿]이 위치하는 디렉토리가 있다면 그것을 추가시켜야 합니다.
