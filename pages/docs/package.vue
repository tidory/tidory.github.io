...
app-page(:active="$store.state.menu.advanced.package.page" :title="$store.state.menu.advanced.package.label")
  ...
  main.text(slot="text")
    article.paragraph
      h2 티도리 패키지
      p 프레임워크의 또 하나의 특징은, #[b 다른 사람이 작성한 코드]를 프로젝트에 포함 시킬 수 있다는 점입니다. 티도리 프레임워크 또한 패키지를 작성하는 것으로 #[b 다른 사람에게 자신의 코드를 제공]할 수 있습니다.
  ...
  footer.footer(slot="footer")
    .arrows
      .left: a(:href="$store.state.menu.advanced.framework.href") #[i.fas.fa-angle-left] {{ $store.state.menu.advanced.framework.label }}
</template>
